...
const MainStoreCategory = () => {
  ...
  const fetchCategories = async () => {
    setLoading(true);
    try {
      const res = await client.get('/main-store/categories');
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await client.put(`/main-store/categories/${editingId}`, formData);
      } else {
        await client.post('/main-store/categories', formData);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await client.delete(`/main-store/categories/${id}`);
      fetchCategories();
    } catch (err) {
    ...
    <div style={{ padding: '20px', background: '#f4f6f9', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
        <h2 style={{ fontSize: '24px', fontWeight: '600', color: '#333' }}>
          {view === 'list' ? 'Manage Category' : editingId ? 'Edit Category' : 'New Category'}
        </h2>
      </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '14px' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderTop: '1px solid #e2e8f0', borderBottom: '2px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '10px 15px', textAlgin: 'center' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan="5" style={{ textAlign: 'center', padding: '20px', color: '#777' }}>No categories found</td></tr>
                ) : (
                  paginated.map((cat, idx) => (
                    <tr key={cat.id || idx} style={{ borderBottom: '1px solid #edf2f7' }}>
                      <td style={{ padding: '10px 15px', color: '#666' }}>{startIndex + idx + 1}</td>
                      <td style={{ padding: '10px 15px', fontWeight: '500', color: '#2d3748' }}>{cat.name}</td>
                      <td style={{ padding: '10px 15px' }}>
                        <span style={{ color: cat.status === 'Active' ? '#38a169' : '#e53e3e', fontWeight: '600' }}>
                          {cat.status || 'Active'}
                        </span>
                      </td>
                      <td style={{ padding: '10px 15px', color: '#718096' }}>
                        {cat.created_at ? new Date(cat.created_at).toLocaleString() : '27-02-2023 01:33:59 AM'}
                      </td>
                      <td style={{ padding: '10px 15px' }}>
                        <button onClick={() => handleEdit(cat)} title="Edit" style={{ background: 'none', border: 'none', color: '#3182ce', cursor: 'pointer', marginRight: '10px', fontSize: '15px' }}>✏️</button>
                        <button onClick={() => handleDelete(cat.id)} title="Delete" style={{ background: 'none', border: 'none', color: '#e53e3e', cursor: 'pointer', marginRight: '10px', fontSize: '15px' }}>🗑️</button>
                      </td>
                    </tr>
                  ))
                )}
