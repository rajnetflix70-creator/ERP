  ...
  return db.transaction(async (trx) => {
    ...
    if (data.items && data.items.length > 0) {
      data.items.forEach(i => {
        total_amount += parseFloat(i.qty_ordered) * parseFloat(i.unit_price);
      });
    }
    ...
    if (data.items && data.items.length > 0) {
      const items = data.items.map(i => ({
        ...
        total: parseFloat(i.qty_ordered) * parseFloat(i.unit_price)
      }));
